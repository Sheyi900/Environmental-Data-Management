...
which(nums > 10)
```
This operation is equivelent to:
...
offd <- rw != cl
```
We now have off-diagnal and diagnal elements:
```{r}
z[offd]
z[!offd]
```
...
For more complext tasks, there are two groups of functions. For array data (vectors, matrices, arrays), we can use `apply, sweep, mapply, sapply`, and `lapply`.  For data frame, we use `aggregate`, `by` (which is a wrapper for `tapply`).
...
- Waht is the nature of the data to be operated on?
- What is the desired end result?

*Groups defined by a list element.* We use `sapply` or `lapply`.  `lapply` always retuns a list, `sapply` may simplify its output into vectors or matrix when appropriate.
...
If we are interested in calculating scalar statistics (e.g., mean, variance), we often use `aggregate` because it reurns a data frame.  If the result is a vector, `tapply` is often used.  
...
Frequently, groups are defined by mulitple variables (e.g., see `ChickWeight` and `iris`).  If we are interested in calculating a scalar summary statistics, we can use `aggregate`
```{r}
